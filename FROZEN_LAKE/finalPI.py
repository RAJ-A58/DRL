    ...
    for s in range(nS):
        ...
        for a in range(nA):
            for prob, next_state, reward, done in P[s][a]:
                action_values[a] =action_values[a]+ prob * (reward + gamma * value_from_policy[next_state])
...
def policy_iteration(P, nS, nA, gamma=0.9, tol=10e-3):
    ...
    while True:
        ...
        new_policy, policy_stable = policy_improvement(P, nS, nA, value_function, policy, gamma)
        if policy_stable:
            break
        policy = new_policy
...
def render_single(env, policy, max_steps=100):
    ...
    for t in range(max_steps):
        ...
        time.sleep(0.25)
        a = policy[ob]
        ob, rew, done, truncated ,_= env.step(a)       # remove a blank after done if on Colab 
        episode_reward += rew
        if done:
            break
